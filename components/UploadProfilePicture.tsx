...
export const UploadProfilePicture: React.FC<UploadProfilePictureProps> = ({ userId }) => {
  ...
  return (
    <div onClick={(e) => e.stopPropagation()}>
      ...
      <label htmlFor="file-upload" className='cursor-pointer sm:py-3 flex gap-2 items-center'>
       <span className='border border-1 px-6 py-2 sm:px-8 sm:py-3 rounded-full sm:font-semibold'>{loading ? `Uploading... ${uploadProgress}%` : 'Change your Avatar'}</span>
      </label>
      {loading && (
